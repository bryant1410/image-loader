...
public class FadingImageSetter implements ImageSetter {
    ...
    private void runTransition(ImageView imageView, Bitmap bitmap) {
        ...
        if (imageView instanceof CustomDrawableCreator) {
            CustomDrawableCreator customDrawableCreator = (CustomDrawableCreator) imageView;
            newDrawable = customDrawableCreator.createCustomDrawableFrom(bitmap);
        } else {
            newDrawable = new BitmapDrawable(resources, bitmap);
        }
